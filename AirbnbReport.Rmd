---
title: "Edx Capstone - Airbnb Analysis Report"
author: "Eshna Airon"
...
The models I have applied here are Linear Regression  , Partial Least-Squares Regression (PLS),
Boosted Generalized Linear Model ,Recursive Partitioning and Regression Trees, Pruned Tree Models, Bagged CART ,Random Forest ,Stochastic Gradient Boosting ,KNN ,Ridge Regression ,Lasso Regression
...
There are 48895 observations which has data in 16 columns.
Both categorical and quantitative data types could be observed.
Each row represents details about lodgings in NYC.
...
MAE.rpart
```
Mean Absolute Error After Pruining
```{r, echo=FALSE, include=TRUE}
...
a = ggplot(mpc, aes(x = fct_reorder(Model, Model.R2), Model.R2))  +
  geom_bar(stat = "identity", fill = "#C7E9B4", col = "black", lwd = 1) +
...
# Conclusion 
The objective of this project was to use machine learning models to predict the Airbnb Prices and understand what features were important to explain the transit. In this report, we tried many models, created some new features, we selected Random Forest model betcause it has the best performance. The final model obtained an RMSE of 161.849 and MAE of 46.501. The most important feature to explain price is neighbourhood group .
